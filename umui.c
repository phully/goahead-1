...
static int    aspGetCurrentUser(int eid, webs_t wp, int argc, char_t **argv);
...
void formDefineUserMgmt(void)
{
	...
	websAspDefine(T("GetCurrentUser"), aspGetCurrentUser);
	websFormDefine(T("user"), formUser);
...
/******************************************************************************/
/*
*  Generate HTML to show current user
*/
...
static int aspGetCurrentUser(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	nBytesSent = websWrite(wp, T("<font color=\"#FF6600\">%s</font>"), wp->userName);

	nBytesSent += websWrite(wp, T("<input type=hidden name=user value=%s>"), wp->userName);
...
static void formUser(webs_t wp, char_t *path, char_t *query)
{
	char_t	*userid, *pass1, *pass2, *group;
	char_t  *pszOperate = NULL;
	char_t  *pszOldPass = NULL;
	int	nCheck;

	a_assert(wp);
	
	pszOperate = websGetVar(wp, T("Operate"), T(""));
	if (0 == gstricmp(pszOperate, T("add"))) {
			...
			if (gstrcmp(pass1, pass2) != 0) {
				websWrite(wp, T("Confirmation Password did not match."));
				websDone(wp, 200);
				return;
			}
			
			nCheck = umAddUser(userid, pass1, group, 0, TRUE);
			if (nCheck != 0) {
				char_t * strError;
				...
				websWrite(wp, T("Unable to add user, \"%s\".  Unable to save user configuration."),
					userid);
			} else {
	...
	else if (0 == gstricmp(pszOperate, T("del"))) {
		userid = websGetVar(wp, T("user"), T("")); 
		...
		if (umUserExists(userid) == FALSE) {
			websWrite(wp, T("ERROR: User \"%s\" not found"), userid);
				...
				websWrite(wp, T("Unable to delete user, \"%s\".  Unable to save user configuration."),
					userid);
		} 
		else {
	...
	else if (0 == gstricmp(pszOperate, T("modifypass"))) {
			userid = websGetVar(wp, T("user"), T(""));
			pszOldPass = websGetVar(wp, T("oldpassword"), T(""));  
			pass1 = websGetVar(wp, T("password"), T("")); 
			pass2 = websGetVar(wp, T("passconf"), T("")); 
		...
		else if (gstricmp(umGetUserPassword(userid), pszOldPass) != 0) {
			websWrite(wp, T("ERROR: user or old password is not right.\n"));
		}
		else if (gstrcmp(pass1, pass2) != 0) {
			websWrite(wp, T("Confirmation Password did not match."));
		} else if (umSetUserPassword(userid, pass1) != 0) {
			websWrite(wp, T("ERROR: Modify Password failed.\n"));
		} else if (umCommit(NULL) != 0) {
			websWrite(wp, T("Unable to modify user:%s password.  Unable to save user configuration."),
				userid);
		}else {
			websWrite(wp, T("modify password success\n"), userid);
		}
	}
...
static int aspGenerateUserList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	while (userid) {
		...
		nBytesSent += websWrite(wp, T("<td align=\"center\" class=\"lefttd\">%s</td>\n"), userid);
		
		pszUserGroup = umGetUserGroup(userid);
		nBytesSent += websWrite(wp, T("<td align=\"center\" class=\"midtd\">%s</td>\n"), pszUserGroup);
		
		if (TRUE == umGetUserEnabled(userid)) {
			nBytesSent += websWrite(wp, T("<td align=\"center\" class=\"midtd\">º§ªÓ</td>\n"));
		}
		else {
			nBytesSent += websWrite(wp, T("<td align=\"center\" class=\"midtd\">Õ£”√</td>\n"));
		}
		
		nBytesSent += websWrite(wp, T("<td align=\"center\" class=\"righttd\">\n"));
		nBytesSent += websWrite(wp, T("<img src=\"img/anshanchu.gif\" alt=\"…æ≥˝\" title=\"…æ≥˝\" style=\"cursor:hand\" onclick=\"return del('%s')\">\n"), 
			userid);
...
static int aspGenerateGroupList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	nBytesSent = 0;
	nBytes = websWrite(wp, 
		T("<SELECT NAME=\"group\" TITLE=\"Select a Group\">"));
